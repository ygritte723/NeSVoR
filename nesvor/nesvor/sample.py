...
def sample_volume(model: INR, mask: Volume, args: Namespace) -> Volume:
    ...
    img = mask.resample(args.output_resolution, transformation)
    ...
    with torch.no_grad():
        for i in range(0, xyz.shape[0], batch_size):
            ...
            xyz_batch = model.sample_batch(
                ...
                None,
                resolution2sigma(
                    args.output_resolution * args.output_psf_factor, isotropic=True
                ),
                0 if args.output_psf_factor <= 0 else args.n_inference_samples,
            )
...
def sample_slice(model: INR, slice: Slice, mask: Volume, args: Namespace) -> Slice:
    ...
    if m.any():
        xyz_masked = model.sample_batch(
            ...
            resolution2sigma(
                slice_sampled.resolution_xyz * args.output_psf_factor, isotropic=False
            ),
            0 if args.output_psf_factor <= 0 else args.n_inference_samples,
        )
